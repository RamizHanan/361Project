...
namespace COMPE361_Project
{
    ...
    public sealed partial class EditSchedule : Page
    {
        ...
        protected override async void OnNavigatedTo(NavigationEventArgs e)
        {
            ...
            {
                EmployeeScheduleList.Items.Add(new ListViewItem { Content = "No Employees" });
            }
        }
        ...
        private void StartTimeSelect(object sender, RoutedEventArgs e)
        {
            ...
            if (End.Text != "")
            {
                string start = x.Text.Replace(":", string.Empty);
                string end = End.Text.Replace(":", string.Empty);
                if (Int32.Parse(end) < Int32.Parse(start))
                {
                    Output.Text = "Cannot Schedule Start Time After End Time";
                }
                else Start.Text = x.Text;
            }
            else Start.Text = x.Text;
        }
        private void EndTimeSelect(object sender, RoutedEventArgs e)
        {
            ...
            if (Start.Text != "")
            {
                string start = Start.Text.Replace(":", string.Empty);
                string end = x.Text.Replace(":", string.Empty);
                if (Int32.Parse(end) < Int32.Parse(start) || Start.Text == "")
                {
                    Output.Text = "Cannot Schedule End Time before Start Time";
                }
                else End.Text = x.Text;
            }
            else Output.Text = "Please Enter a Start Time First";
        }
        private async void Schedule(object sender, RoutedEventArgs e)
        {
            ...
            else
            {
                ...
                {
                    Output.Text = "Employee Does Not Exist";
                }
            }
        }
